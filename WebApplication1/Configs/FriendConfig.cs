...
namespace ConsoleApp1.Configs
{
    public class FriendConfig : BaseConfig<Friend>
    {
        ...
        public override void Configure(EntityTypeBuilder<Friend> builder)
        {
            ...
            builder.HasOne(x => x.Invetee).WithMany().HasForeignKey(x => x.InviteToFk).OnDelete(DeleteBehavior.NoAction);
            builder.HasOne(x => x.Inviter).WithMany().HasForeignKey(x => x.InviteByFk).OnDelete(DeleteBehavior.NoAction);
        }
    }
}
